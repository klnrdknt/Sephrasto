...
class DatenbankEdit(object):
    ...
    def wiederherstellen(self):
        for itm in self.ui.listDatenbank.selectedIndexes():
            ...
            if exists:
                ...
                self._add_standard_buttons_to_warning_window(messageBox)
                return
    ...
    def saveDatenbank(self):
        ...
        if not isInRulesPath:
            infoBox = self._open_user_prompt(
        ...
        if spath == refDatabaseFile:
            self._database_overwrite_error_window()
            self.saveDatenbank()
            return
    ...
    def _open_user_prompt(self, question: str, title: str, answer2: str):
        result = QtWidgets.QMessageBox()
        result.setIcon(QtWidgets.QMessageBox.Question)
        result.setText(question)
        result.setWindowTitle(title)
        result.addButton(QtWidgets.QPushButton("Ja"), QtWidgets.QMessageBox.YesRole)
        result.addButton(QtWidgets.QPushButton(answer2), QtWidgets.QMessageBox.NoRole)
        return result
    ...
    def _database_overwrite_error_window(self):
        infoBox = QtWidgets.QMessageBox()
        infoBox.setIcon(QtWidgets.QMessageBox.Information)
        infoBox.setText("Überschreiben der zentralen Datenbank verhindert!")
        infoBox.setInformativeText(
            "Bitte schreibe deine eigenen Regeln nicht mit in die zentrale datenbank.xml - \\\x1fdiese wird bei vielen Updates von Sephrasto verändert, wodurch du deine Regeln \\\x1fverlieren würdest. Stattdessen kannst du sie als separate Datei abspeichern und in den Einstellungen selektieren. \\\x1fSie wird dann automatisch beim Programmstart mit geladen. Änderungen darin überschreiben \\\x1fdie datenbank.xml, aber bleiben bei Updates erhalten!"
        )

        infoBox.setWindowTitle("Ungültige Datei!")
        self._add_standard_buttons_to_warning_window(infoBox)
    ...
    def _init_database_window(self):
        self.datenbank.xmlLaden()
        self._update_types_in_gui()
        self.changed = False

    def _update_types_in_gui(self):
        self.initDatabaseTypes()
        self.updateGUI()
        self.updateWindowTitleAndCloseButton()

    def _add_standard_buttons_to_warning_window(self, widget):
        widget.setStandardButtons(QtWidgets.QMessageBox.Ok)
        widget.setEscapeButton(QtWidgets.QMessageBox.Close)
        widget.exec_()
