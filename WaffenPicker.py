...
class WaffenPicker(object):
        ...
        for kind in kampfferts:
            ...
            for waf in Wolke.DB.waffen:
                if self.ui.nameFilterEdit.text():
                    ...
                    if filterText not in Wolke.DB.waffen[waf].anzeigename.lower():
                        continue
    ...
    def changeHandler(self):
        kampfferts = [fert.name for fert in self.findKampfFertigkeiten()]
        self.current = ""
    ...
    def updateInfo(self):
        ...
        if self.current == "":
            ...
            tp = str(w.W6) + " W6"
            tp += " " + str(w.plus) if w.plus < 0 else " +" + str(w.plus)
