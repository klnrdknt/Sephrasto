...
class EquipWrapper(QtCore.QObject):
    ...
    def __init__(self):
        ...
        for el in range(1, 9):
            ...
            for el2 in kampfstile:
                getattr(self.uiEq, "comboStil" + str(el)).addItem(el2)
    ...
    def refreshKampfstile(self, index):
        ...
        if name != "" or anzeigename != "":
            if name in Wolke.DB.waffen:
                entries = [Definitionen.KeinKampfstil]
                ...
                for el in entries:
                    eval("self.uiEq.comboStil" + str(index + 1) + f".addItem({el})")
                if self.initialLoad:
    ...
    def loadWeaponIntoFields(self, W, index):
        ...
        eval(
            "self.uiEq.edit"
            + Warr[count]
            + f"eig.setText({', '.join(W.eigenschaften)})"
        )
        eval(
            "self.uiEq.edit"
            + Warr[count]
