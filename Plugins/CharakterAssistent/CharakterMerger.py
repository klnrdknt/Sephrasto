...
class CharakterMerger(object):
    ...
    def handleChoices(paths, title, geschlecht, spezies, kultur, profession):
        ...
        for variantListCollection in variantListCollections:
            ...
            if not variantListCollection.choiceLists:
                continue
            ...
            for index in choices:
                ...
                if choiceList.beschreibung is None:
        ...
        if description:
            ...
            if spezies:
                char.rasse = (
                    description if anyChooseOne else title + " (" + description + ")"
                )
            elif kultur:
    ...
    def xmlNodeToChoices(choiceListCollection, node):
        for child in node:
            ...
            if "geschlecht" in child.attrib:
                ...
                if geschlecht not in ["männlich", "weiblich"]:
                    logging.warn(
            ...
            for fer in child.findall("Attribut"):
                ...
                if choice.name not in Definitionen.Attribute.keys():
                    logging.warn(
            ...
            for fer in child.findall("Fertigkeit"):
                ...
                if choice.name not in Wolke.DB.fertigkeiten:
                    logging.warn(
            ...
            for fer in child.findall("Übernatürliche-Fertigkeit"):
                ...
                if choice.name not in Wolke.DB.übernatürlicheFertigkeiten:
                    logging.warn(
            ...
            for vor in child.findall("Vorteil"):
                ...
                if choice.name not in Wolke.DB.vorteile:
                    logging.warn(
            ...
            for vor in child.findall("Talent"):
                ...
                if choice.name not in Wolke.DB.talente:
                    logging.warn(
    ...
    def applyChoiceToChar(choice):
        ...
        if choice.typ == "Eigenheit":
            if len(char.eigenheiten) == 8:
                return
            if choice.name not in char.eigenheiten:
                char.eigenheiten.append(choice.name)
        elif choice.typ == "Attribut":
            if choice.name not in char.attribute:
                return
            ...
            for _name, fert in char.fertigkeiten.items():
                if choice.name in fert.gekaufteTalente:
                    return
            for _name, fert in char.übernatürlicheFertigkeiten.items():
                if choice.name in fert.gekaufteTalente:
                    return
