...
class TalentPicker(object):
    def __init__(self, fert, ueber):
        ...
        for el in Wolke.DB.talente:
            ...
            if fert in talent.fertigkeiten and Wolke.Char.voraussetzungenPrüfen(
                ...
                ) and el not in self.talenteVariable:
            ...
            for i in range(self.rowCount):
                ...
                if self.model.item(i).checkState() == QtCore.Qt.Checked:
                    for el in Wolke.DB.talente[tmp].fertigkeiten:
                        if el in self.refC and tmp not in self.refC[el].gekaufteTalente:
                    ...
                    for el in Wolke.DB.talente[tmp].fertigkeiten:
                        if el in self.refC and tmp in self.refC[el].gekaufteTalente:
    ...
    def setVariableKosten(self, talent, kosten, kommentar):
        ...
        if talent not in self.talenteVariable:
            vk = VariableKosten()
    ...
    def updateFields(self, talent):
        ...
        if (
            self.fert == "Gebräuche"
            and self.displayStr(Wolke.DB.talente[talent].name) == Wolke.Char.heimat
        ):
